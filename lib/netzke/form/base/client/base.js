      ...
      if (this.getForm().hasUpload()) {
        ...
        this.getForm().submit({ // normal submit
          ...
          failure: function(form, action){
            if (action.response == undefined) { //TODO: Review why its failing while submitting the tree form contains file upload.
              t.server.submit(Ext.apply((t.baseParams || {}), { data:Ext.encode(values) }), function(){ t.setLoading(false); });
              return;
            }
